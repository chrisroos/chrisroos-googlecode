...
class NationalRailEnquiries
  ...
  class Parameters
    ...
    def initialize(msg)
      @depart_station, @arrive_station, depart_date, depart_time = msg.split(' ')
      @depart_day, @depart_month = depart_date.split('-')
      @depart_hour, @depart_minute = depart_time.split(':')
    end
  end
  ...
  def search
    curl_cmd = <<-EndCurl
/usr/local/bin/curl http://ojp1.nationalrail.co.uk/en/pj/jp \
-X"POST" \
...
-s
    ...
    `#{curl_cmd}`
    html = `/usr/local/bin/curl http://ojp1.nationalrail.co.uk/en/pj/tt -b"#{COOKIE_JAR}" -s`
